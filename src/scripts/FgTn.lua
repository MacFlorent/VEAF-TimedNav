...
local Id = "FgTn"
local function LogError(oLog)
    Fg.LogError(oLog, Id)
end
local function LogInfo(oLog)
    Fg.LogInfo(oLog, Id)
end
local function LogDebug(oLog)
    Fg.LogDebug(oLog, Id)
end
...
function FgTnTools.OutTextForGroup(mooseGroup, sText, sSoundFile, iDelay)
    if (not Fg.IsNullOrEmpty(sSoundFile) and not Fg.IsNullOrEmpty(FgTn.SoundFolder)) then
        sSoundFile = FgTn.SoundFolder .. "/" .. sSoundFile
    end

	Fg.MessageForGroup(mooseGroup, sText, sSoundFile, 0)
    if (Debug) then
        trigger.action.outText(mooseGroup.GroupName .. " DEBUG > " .. sText, 59, false)
        
        if (not Fg.IsNullOrEmpty(sSoundFile)) then
            trigger.action.outSound(sSoundFile)
        end
    end
end
...
function FgTnTools.GetRandomSpeedMs(mooseGroup)
    ...
    if (iMaxSpeed > iUpThreshold) then
        iMaxSpeed = iUpThreshold
    end
...
function FgTnTools.TaskGroupToZone(mooseGroup, mooseZone, iSpeedKmh, iAltitudeM, bLand)
    if (mooseGroup:GetPlayerCount() <= 0) then
        ...
        if (bLand) then
            pointType = POINT_VEC3.RoutePointType.Land
        end
        local pointAction = POINT_VEC3.RoutePointAction.TurningPoint
        if (bLand) then
            pointAction = POINT_VEC3.RoutePointAction.Landing
        end
...
function FgTnWaypoint:ToString()
    local sZone = "-no zone-"
    if (self.MooseZone) then
        sZone = self.MooseZone.ZoneName
    end
...
function FgTnWaypointList.ToString()
    local sString = ""
    if (FgTnWaypointList.LastStage) then
        sString = sString .. "\nLastStage=" .. FgTnWaypointList.LastStage
    end
    if (FgTnWaypointList.RefuelWaypoint) then
        sString = sString .. "\nRefuelWaypoint=[" .. FgTnWaypointList.RefuelWaypoint:ToString() .. "]"
    end
...
function FgTnLeg:ToString()
    ...
    if (self.WaypointTo) then
        sWpTo = "[" .. self.WaypointTo:ToString() .. "]"
    end
...
function FgTnLeg:OutTextForGroup(sText, sSound, iDelay)
    FgTnTools.OutTextForGroup(self.MooseGroup, sText, sSound, iDelay)
end
...
function FgTnLegList.EndNavigation(leg)
    if (FgTnWaypointList.Destinations and #FgTnWaypointList.Destinations > 0) then
        ...
        leg.markId = nil
        leg:OutTextForGroup("Navigation completed\nYour final destination for landing is " .. zoneAirbase.ZoneName, FgTn.SoundComplete)
    ...
    if (not bSkip) then
        ...
        if (leg.TimeTo and leg.TimeTo > 0) then
            ...
            if (legTime < -timeTolerance) then
                onTime = 1
                leg:OutTextForGroup("You are LATE", FgTn.SoundFailure)
                -- leg:FlareZone(FLARECOLOR.Red)
            elseif (legTime > timeTolerance) then
                onTime = -1
                leg:OutTextForGroup("You are EARLY", FgTn.SoundFailure)
                -- leg:FlareZone(FLARECOLOR.Red )
            else -- on time
                onTime = 0
                leg:OutTextForGroup("You are ON TIME", FgTn.SoundSuccess)
...
---------------------------------------------------------------------------------------------------
--- Parameters table can contain:
---	NavGroupNames = nil, -- list of groups that will trigger the timed navigation
---	NavGroupAddPlayers = false, -- if true, playable groups will be added as triggers to the timed navigation
---	EndingAction = nil, -- optional function to execute when the timed navigation is ended
---	StartZoneName = "FgTnStartZone", -- editor name of the zone that will start the timed navigation
---	WpZoneRadius = 2500, -- radius in meters to validate a waypoint passage
---	LastStage = nil, -- number of the last stage
---	Destinations = {}, -- optional, airbases that will be proposed as last waypoint
---	RefuelEndStage = nil, -- optional, number of the stage used for a refuel stop
---	RefuelEndZoneName = nil, -- optional, editor name of a zone that will end the refuel stop and resume the timed navigation
---	AdditionalData = nil -- optional, table of additional data for the stages of the timed navigation
---	SoundFolder = nil, -- optional, folder (in miz file) where the sound files are stored
---	SoundStart = nil, -- optional, sound file for the start of the navigation
---	SoundComplete = nil, -- optional, sound file for the completion of the navigation
---	SoundSuccess = nil, -- optional, sound file for the success of a leg
---	SoundFailure = nil, -- optional, sound file for the failure of a leg
---
---------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------
FgTn = {
...
function FgTn.Start(parameters)
	...
	if (parameters.NavGroupAddPlayers == true) then
		for sClientName, mooseClient in pairs(_DATABASE.CLIENTS) do
			local sGroupName = _DATABASE.Templates.Units[sClientName].GroupName
			if (not Fg.Contains(FgTn.NavGroupNames, sGroupName)) then
				LogDebug("Add playable group " .. sGroupName)
				table.insert(FgTn.NavGroupNames, sGroupName)
			end
		end		
	end
	
    LogDebug("FgTn.NavGroupNames=" .. Fg.ToString(FgTn.NavGroupNames))

    FgTn.SoundFolder = parameters.SoundFolder
    FgTn.SoundStart = parameters.SoundStart
    FgTn.SoundComplete = parameters.SoundComplete
    FgTn.SoundSuccess = parameters.SoundSuccess
    FgTn.SoundFailure = parameters.SoundFailure
    ...
    FgTn.Scheduler = SCHEDULER:New(nil, function()
